...
### ROI가 뭐냐
**Return on Investment** — 이 작업에 1~1.5일 쓰면 얼마나 돌려받냐.

- **투자**: 1~1.5 작업일 (next-intl 세팅 + 22개 파일, ~160개 문자열 추출)
- **회수**: 지금 사용자 = 한국어 1명. 영어 UI 실제 쓰기 전까지 체감 이득 **0**
- **같은 1~1.5일 → UX(토스트·진행률·Web Worker)**에 투자하면 매 세션 즉시 체감
- **결론**: 지금은 기회비용이 이득보다 크다 → 뒤로 미룸

### 리스크 3줄
1. **JSX 깨짐 가능성** — 문자열이 `<span>` / `className` / `aria-label` / 템플릿 리터럴에 박혀 있어서 치환 중 태그 구조 실수 가능
2. **UI 테스트 부재** — 컴포넌트 테스트가 없어 문자열 누락이 조용히 프로덕션으로 감
3. **라우트 재구성 부작용** — `app/` → `app/[locale]/` 이동이 `project-menu`·`studio-app`의 경로 가정을 건드림

### 결정 트리거
이 중 하나라도 발생하면 이 계획 실행:
- 영어권 사용자/파일럿 고객 생김
- 마케팅이 영문 랜딩 요구
- UX·품질 부채 소진되어 다음 투자처 필요

### 시작할 때 할 일
1. `docs/i18n-plan.md` 이 문서 열기
2. §5 실행 순서대로 진행
3. 의존성: `pnpm add next-intl` 1개만
