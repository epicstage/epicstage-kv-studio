...
// CI-specific analysis. Output is substituted for the CI image itself when
// calling OpenAI GPT Image 2 — the goal is to transfer palette/tone/style
// influence without piping the logo pixels into an /images/edits call that
// would otherwise reproduce the logo in the output.
//
// HARD RULE inside the prompt: analyze color/tone/style only; never describe
// the logo's shape, wordmark, letterforms, or any mark that could be
// reconstructed from text alone.
export const ANALYZE_CI_SYSTEM = `너는 브랜드 CI 분석가다.
첨부된 CI 자료(로고/브랜드 북)에서 **색·톤·그래픽 질감·타이포 인상**만 추출하라.
로고의 형태/심볼/워드마크/글자 모양은 절대 묘사하지 마라 (묘사하면 다운스트림 이미지 모델이 로고를 재현한다).

출력 스키마 (JSON만):
{
  "palette_hex": ["#RRGGBB", "#RRGGBB", "#RRGGBB"],
  "palette_usage": "주색·보조색·강조색이 어떤 비중과 용도로 쓰이는지 한 문장",
  "visual_tone": "전체적인 톤&매너 (예: 차분/다이내믹/미니멀/클래식 등) 한 문장",
  "graphic_character": "그래픽 질감 (예: 라운드/샤프, 기하/유기, 두꺼움/얇음) 한 문장 — 로고 형태 묘사 금지",
  "typography_feel": "타이포 인상 (예: 모던 산세리프/슬랩 세리프) 한 문장 — 실제 글자 모양 복제하지 말 것"
}`;
